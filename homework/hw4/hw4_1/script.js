...
function areaOfARectangleFunction(a, b) {
    return a * b
}
...
function areaOfACircleFunction(radius) {
...
function areaOfACylinderFunction(radius, height) {
...
function workWithArrayFunction(array) {
...
function createParagraphWithTextFunction(text) {
...
function createLiFunction(name) {
    document.write(`<ul>
                         <li>${name}</li>
                         <li>${name}</li>
                         <li>${name}</li>
...
function createSomeQuantityFunction(name, quantity) {
    let html = `<ul>`
...
function createListOfArrayItems(array) {
    let html = `<ul>`
    for (let i = 0; i < array.length; i++) {
        html += `<li>${array[i]}</li>`
    }
...
function workWithObjectsInArrayFunction(array) {
...
workWithObjectsInArrayFunction(users)

console.log(`– створити функцію яка повертає найменьше число з масиву`)

function smallestNumberFunction(numbers) {
...
function sumOfArrayNumbersFunction(arr) {
    let sum = 0;
...
function changeArrayByIndexFunction(array, previousIndex, newIndex) {
...
function exchangeMoneyFunction(sumUAH, currencyValues, exchangeCurrency) {
    let sum = 0;
