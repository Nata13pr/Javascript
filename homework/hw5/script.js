...
let areaOfARectangleArrowFunction = (a, b) => a * b;
...
let areaOfACircleArrowFunction = (radius) => Math.PI * Math.pow(radius, 2)
...
let areaOfACylinderArrowFunction = (radius, height) => 2 * Math.PI * radius * (radius + height)
...
let workWithArrowFunction = (array) => {
...
workWithArrowFunction(productsArray)
...
let createParagraphWithTextArrowFunction = (text) => document.write(`<p>${text}</p>`);
...
let createLiArrowFunction = (name) => {
...
let createSomeQuantityArrowFunction = (name, quantity) => {
    let html = `<ul>`
...
let createListOfArrayArrowItems = (array) => {
    let html = `<ul>`
    for (let i = 0; i < array.length; i++) {
        html += `<li>${array[i]}</li>`
    }
...
const literalMixedArray = [77, -15, 4.02, 1, 1000, "Яблуко", "Осінь", "Код", "Canvas", "JavaScript", true, false, true, false, true, 555.55, "Змішаний тип", false, 99, "20-й елемент"];
createListOfArrayArrowItems(literalMixedArray)
...
let workWithObjectsInArrayArrowFunction = (array) => {
...
workWithObjectsInArrayArrowFunction(users)

console.log(`– створити функцію яка повертає найменьше число з масиву`)

let smallestNumberArrowFunction = (numbers) => {
...
console.log(smallestNumberArrowFunction(uniqueNumbersArray));
...
let sumOfArrayNumbersArrowFunction = (arr) => {
    let sum = 0;
...
console.log(sumOfArrayNumbersArrowFunction(positiveNumbersArray))

console.log(`– створити функцію swap(arr,index1,index2). Функція міняє місцями значення у відповідних індексах

Приклад  swap([11,22,33,44],0,1) //=> [22,11,33,44]`)

let changeArrayByIndexArrowFunction = (array, previousIndex, newIndex) => {
...
let swapedArray = [9, 23, 45, 12, 7, 88, 30, 15, 61];
console.log(swapedArray);

console.log(changeArrayByIndexArrowFunction(swapedArray, 5, 0));

console.log(`Написати функцію обміну валюти exchange(sumUAH,currencyValues,exchangeCurrency)

Приклад exchange(10000,[{currency:’USD’,value:25},{currency:’EUR’,value:42}],’USD’) // => 400`)

let exchangeMoneyArrowFunction = (sumUAH, currencyValues, exchangeCurrency) => {
    let sum = 0;
...
console.log(exchangeMoneyArrowFunction(5555, exchangeRates, 'CAD'));