...
let lengthOfStringArrowFunction = (string) => console.log(`Довжина строки ${string} -`, string.length)
...
let convertToUpperCaseArrowFunction = (string) => string.toUpperCase();
...
let convertToLowerCaseArrowFunction = (string) => string.toLowerCase();
...
let toDeleteExtraSpaces = (string) => {
...
let convertToString = arrayNumbers.map(number => number.toString())
...
let filteredArray = coursesAndDurationArray.filter(number => number.monthDuration > 5)

console.log(filteredArray)

// -------------------------- — за допомоги map перетворити кожен елемент на наступний тип {id,title,monthDuration}`)--------------

let mappedArray = coursesAndDurationArray.map((number, index) => {
...
let filterArrayWithSass = coursesArray.filter(obj => obj.modules.includes('sass'))

console.log(filterArrayWithSass)

let filterArrayWithDocker = coursesArray.filter(obj => obj.modules.includes('docker'))
...
let findSpadeAce = cards.find(card => card.value === 'ace' && card.cardSuit === 'spade');

console.log(findSpadeAce)

console.log(`– всі шістки`)

let findAllSixths = cards.filter(card => card.value === '6');

console.log(findAllSixths)

console.log(` – всі червоні карти`)

let findAllRedCards = cards.filter(card => card.color === 'red');

console.log(findAllRedCards)

let findAllDiamonds = cards.filter(card => card.cardSuit === 'diamond');

console.log(findAllDiamonds)

console.log(`  – всі трефи від 9 та більше`)

let findAllClubsFromNine = cards.filter(card => card.cardSuit === 'clubs' && card.value === '9' || card.cardSuit === 'clubs' && card.value === '10' || card.cardSuit === 'clubs' && card.value === 'jack' || card.cardSuit === 'clubs' && card.value === 'queen' || card.cardSuit === 'clubs' && card.value === 'king' || card.cardSuit === 'clubs' && card.value === 'ace');
