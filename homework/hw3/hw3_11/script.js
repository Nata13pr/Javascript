...
let mixedArray = [true, 4, 'spring', false, 99, 'number', 555, false, 'string', -3];

for (i = 0; i < mixedArray.length; i += 1) {
...
let anotherMixedArray = ['one', true, 999, 'two', -8, 'three', false, 'spy', true, 777];

let x = 0;

while (x < anotherMixedArray.length) {
    if (typeof anotherMixedArray[x] === "boolean") {
        console.log(anotherMixedArray[x]);
    }
    x += 1;
}
...
let secondMixedArray = ['autumn', 9987, false, 'summer', 55, true, 'winter', 9, -54, false];

for (z = 0; z < secondMixedArray.length; z += 1) {
...
let thirdMixedArray = ['cat', false, 66, 'spy', 888, true, 'mouse', 0, false, 'dog']
