...
let filteredUsers = usersArray.filter(user => !(user.id % 2))
...
function Car(model, manufacture, year, maxSpeed, engineVolume) {
    ...
    this.info = function () {
        console.log(`Модель - ${this.model};Бренд - ${this.manufacture}; Рік випуску - ${this.year}; Максимальна швидкість - ${this.maxSpeed}; Об'єм  двигуна - ${engineVolume}`)
    }
...
console.log(car.addDriver({name: 'mom', age: 30, gender: 'male'}));
console.log(car)
